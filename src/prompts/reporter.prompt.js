...
## 格式禁止（極重要）
⛔ **絕對不可使用 Markdown 語法**，包括但不限於：
  - 不可用 # ## ### 作為標題（章節標題已由系統生成）
  - 不可用 ** 或 __ 包裹粗體文字
  - 不可用 * 或 _ 包裹斜體文字
  - 不可用 | 和 --- 畫表格（表格請用 [TABLE_JSON] 格式）
  - 不可用 --- 或 *** 畫分隔線
  - 不可用 - 或 * 開頭做項目符號列表
你輸出的是**純文字段落**，直接嵌入 Word 文件。任何 Markdown 符號都會以原始字元顯示在報告中。

## 寫作要求
請直接輸出純文字正文（⛔ 非 JSON、⛔ 非 Markdown — 不可用 #、**、|表格|、--- 等任何 Markdown 語法）：
- 長度：**1200-1800 繁體中文字**（這是專業研報的標準深度，不可偷懶寫短）
- 分為 **5-7 個段落**，每段 **150-250 字**（每段只講一個論點，不可超過 250 字）
- **段落之間必須空一行**（用兩個換行符分隔，這很重要，否則文件無法正確分段）
- 每段開門見山提出論點，再展開分析，最後收束
- 結論章節同樣適用：每個結論/建議獨立成一段，不可全部堆在一起
- 使用台灣商業寫作規範，避免翻譯腔
- 引用數據時標明年份與來源，如「2024 年進口額達 80.3 億美元（國際貿易署）」
